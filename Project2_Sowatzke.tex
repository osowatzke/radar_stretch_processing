...
\end{figure}
\noindent
The fast-time FFT extracts the frequency content in each pulse. Each target will produce a single tone, which results in a single peak. The frequency of each peak is influenced by the range and doppler shift of each target. The doppler shift will produce the following error in range measurements:
\begin{equation}
...
\end{equation}
The LFM radar system sacrifices some of this unambiguous range for a lower sampling rate. The unambiguous range of an LFM radar system is given by the following equation:
\begin{equation}
\label{Rw CPI0}
...
\label{multi_prf_rdm}
\end{figure}
The true velocity and range of the target can be computed by examining the possible ranges and velocities in the unambiguous interval. For the first CPI illustrated in Fig. \ref{RDM3}, the possible target ranges are given as follows:
\begin{table}[H]
...
\end{tabularx}
\end{table}
For the second CPI illustrated in Fig. \ref{multi_prf_rdm}, the possible target ranges are given as follows:
\begin{table}[H]
...
\end{tabularx}
\end{table}
The true range of the target is the closest pair of measurements, which is 200.25m. Note that this result matches the result shown in Fig. \ref{RDM3_resolvable}. 
\par
The velocity of the second target can be found in a similar manner. For the CPI illustrated in Fig. \ref{RDM3}, the possible target velocities are given as follows:
\begin{table}[H]
...
\end{tabularx}
\end{table}
The true velocity of the target is the closest pair of measurements, which is approximately 80m/s. Note that this result approximately matches the result shown in Fig. \ref{RDM3_resolvable}. Any measurement differences occur due different velocity resolutions in each of the CPIs.
